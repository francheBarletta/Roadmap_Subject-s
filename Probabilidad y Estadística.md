# Probabilidad y Estadística — Plan de estudio personal

Final: 29 Junio (1ra fecha)
Prioridad: ALTA para el final

---

## Fechas clave

| Fecha | Evento |
|---|---|
| Dom 29 Junio | Final PyE / Álgebra (1ra fecha) |
| Lun 20 Julio | Final PyE / Álgebra (2da fecha) |
| Mar 11 Agosto | Final PyE / Álgebra (3ra fecha) |

---

## Estado actual (6 Mayo)

- Guías 1 a 4: vistas hace ~4 meses, base parcial — **retomar desde cero**
- Guías 5 a 8: no vistas
- Temas más sólidos: G1–G4 (probabilidad, VA discretas/continuas, distribuciones conjuntas)
- Temas pendientes: G5–G8 (estimación, intervalos de confianza, pruebas de hipótesis)

---

## Guías de ejercicios

| # | Tema | Estado |
|---|---|---|
| G1 | Probabilidad, probabilidad condicional, independencia | [ ] retomar |
| G2 | Variables aleatorias discretas | [ ] retomar |
| G3 | Variables aleatorias continuas | [ ] retomar |
| G4 | Distribuciones conjuntas y muestras aleatorias | [ ] retomar |
| G5 | Estimación puntual — momentos y máxima verosimilitud | [ ] nuevo |
| G6 | Intervalos de confianza (una muestra) | [ ] nuevo |
| G7 | Pruebas de hipótesis (una muestra) | [ ] nuevo |
| G8 | Pruebas de hipótesis (dos muestras) | [ ] nuevo |

---

## Conceptos clave

### Bloque 1 — Repaso (G1–G4)
- [ ] Espacio muestral, eventos, axiomas de probabilidad
- [ ] Probabilidad condicional y Teorema de Bayes
- [ ] Independencia de eventos
- [ ] Variables aleatorias discretas: fmp, fda, esperanza, varianza
- [ ] Distribuciones discretas: Binomial, Poisson, Hipergeométrica, Binomial Negativa
- [ ] Variables aleatorias continuas: fdp, fda, esperanza, varianza
- [ ] Distribuciones continuas: Uniforme, Normal, Exponencial, Gamma, Weibull
- [ ] Distribuciones conjuntas: marginales, covarianza, correlación, independencia
- [ ] TLC y Normal estándar para aproximaciones

### Bloque 2 — Nuevo (G5–G8)
- [ ] Estimadores insesgados y eficientes
- [ ] Método de momentos
- [ ] Máxima verosimilitud (MV)
- [ ] Intervalos de confianza para media (σ conocido y desconocido)
- [ ] Intervalos de confianza para varianza (chi-cuadrado)
- [ ] Intervalos de confianza para proporciones
- [ ] Prueba de hipótesis: H0, Ha, región de rechazo, error tipo I y II
- [ ] Estadístico Z y estadístico T
- [ ] p-valor e interpretación
- [ ] Pruebas con dos muestras: independientes y apareadas

---

## Sesiones de estudio programadas

> PyE tiene días propios completos — puede avanzar más rápido que las otras materias
> Vie: PyE solo (sesión completa)
> Lun: PyE primera (4:30am, antes de ODC y Álgebra)
> Lun 8 Junio: PyE solo ese día
> INV = Sesión de investigación (leer libro + anotar)
> SR = Spaced Repetition
> Libro de referencia: Devore — Probabilidad y Estadística

---

### MAYO — Semana 1 (7 al 11)

**Vie 8** *(sesión completa PyE)*
- INV: Leer probabilidad, axiomas, probabilidad condicional, Teorema de Bayes (HI-PyE-01)
- Ejercicios: G1 ej. 3, 8, 11, 16
- Objetivo: reconectar con el tema

---

### MAYO — Semana 2 (12 al 18)

**Lun 11** *(PyE primera del día)*
- INV: Leer variables aleatorias discretas, fmp, fda, esperanza, varianza (HI-PyE-02)
- Ejercicios: G2 ej. 1, 2, 6

**Vie 15** *(sesión completa PyE)*
- Tema: G2 — Distribuciones discretas: Binomial, Poisson, Hipergeométrica
- Ejercicios: G2 ej. 7, 9, 11, 12, 14
- SR al final: repasar G1 con caja de zapatos

---

### MAYO — Semana 3 (19 al 25)

**Lun 18** *(PyE primera del día)*
- INV: Leer variables aleatorias continuas, fdp, Normal, Exponencial (HI-PyE-03)
- Ejercicios: G3 ej. 1, 2, 3

**Vie 22** *(sesión completa PyE)*
- Tema: G3 — Distribuciones continuas: Normal, Exponencial, otras
- Ejercicios: G3 ej. 5, 6, 7, 11, 12
- SR al final: repasar G2

---

### MAYO — Semana 4 (26 al 31)

**Lun 25** *(PyE primera del día)*
- INV: Leer distribuciones conjuntas, covarianza, correlación, TLC (HI-PyE-04)
- Ejercicios: G4 ej. 1, 2, 3

**Vie 29** *(sesión completa PyE)*
- Tema: G4 — TLC, distribución de medias muestrales, aproximaciones normales
- Ejercicios: G4 ej. 9, 10, 11, 12, 13, 14
- SR al final: repasar G3

---

### JUNIO — Semana 1 (1 al 7)

> ⚠️ Semana crítica — parciales Álgebra y ODC — PyE sigue su ritmo

**Lun 1** *(PyE primera del día)*
- INV: Leer estimación puntual, estimadores insesgados, método de momentos (HI-PyE-05)
- Ejercicios: G5 ej. 1, 2, 3, 4

**Vie 5** *(sesión completa PyE — día del parcial ODC, PyE sigue igual)*
- Tema: G5 — Máxima verosimilitud, propiedad de invarianza
- Ejercicios: G5 ej. 6, 7, 8, 9, 10
- SR al final: repasar G4

**Dom 7** — sin PyE

---

### JUNIO — Semana 2 (15 al 15)

> Arranque desde el Dom 15 de Junio. 6 ejercicios por día, Sáb 28 carga liviana (5 ejercicios), Dom 29 FINAL sin sesión.

**Lun 15**
- INV: Leer variables aleatorias discretas — fmp, fda, distribuciones (HI-PyE-02)
- Ejercicios: G2 ej. 4, 5, 6, 7, 8, 9
*Ej 4* — fmp desde un experimento aleatorio sin reposición
*Ej 5* — fmp con experimento de decisiones secuenciales
*Ej 6* — Esperanza y varianza de una VA discreta: propiedades de E(X)
*Ej 7* — Esperanza y varianza: transformaciones lineales y no lineales de X
*Ej 8* — Esperanza y varianza de VA discreta + costo esperado
*Ej 9* — Distribución Binomial: probabilidades y valor esperado

---

### JUNIO — Semana 3 (18 al 22)

> Arranque desde el Jue 18 de Junio. 7 ejercicios por día (algunos 8), Dom 28 carga liviana (4 ejercicios), Lun 29 FINAL sin sesión.

**Jue 18**
- INV: Leer variables aleatorias discretas — fmp, fda, distribuciones (HI-PyE-02)
- Ejercicios: G2 ej. 11, 12, 13, 14, 15, 17
*Ej 11* — Distribución Binomial: probabilidad acumulada
*Ej 12* — Distribución Hipergeométrica: sin reposición, E(X) y V(X)
*Ej 13* — Distribución Binomial Negativa: número de ensayos hasta r éxitos
*Ej 14* — Distribución de Poisson: probabilidades acumuladas y E(X)
*Ej 15* — Suma de variables de Poisson independientes
*Ej 17* — Esperanza y varianza de Hipergeométrica y Binomial Negativa

**Vie 19**
- INV: Leer distribuciones continuas — Normal, Exponencial, uso de tablas Z (HI-PyE-03)
- Ejercicios: G3 ej. 1, 2, 3
*Ej 1* — Función de densidad (fdp): cálculo de probabilidades e integración
*Ej 2* — FDA a partir de fdp: cálculo de probabilidades con fda
*Ej 3* — fdp con parámetro k: percentiles, E(X), V(X), σ

**Sáb 20** — Sin sesión (no se pudo estudiar)

**Dom 21** —
- Ejercicios: G3 ej. 4, 5, 6
*Ej 4* — Distribución Uniforme: fdp, fda, E(X), V(X), probabilidades ✓
*Ej 5* — Distribución Normal: estandarización y uso de tabla Z ✓
*Ej 6* — Distribución Normal: probabilidades y percentiles aplicados ✓

**Lun 22**
- Ejercicios: G3 ej. 7, 8, 9, 10, 11, 12, 13; G4 ej. 1, 2, 3, 4
*Ej 7* — Distribución Normal inversa: encontrar µ y σ desde percentiles
*Ej 8* — Transformación lineal de Normal: Y = aX + b sigue siendo Normal
*Ej 9* — Combinación de Normales: mezcla de máquinas con distintos parámetros
*Ej 10* — Distribución Normal: probabilidades acumuladas e intervalo simétrico
*Ej 11* — Distribución Exponencial: probabilidades, mediana, propiedad sin memoria
*Ej 12* — Distribución Exponencial: sistema en serie, distribución del mínimo
*Ej 13* — Distribución de Poisson + Exponencial: sistema de dos bombas
- INV: Leer distribuciones conjuntas, covarianza, correlación, TLC (HI-PyE-04)

---

### JUNIO — Semana 4 (23 al 29) → Preparación Final

**Mar 23**
- Ejercicios: G4 ej. 1, 2, 3
*Ej 1* — Distribución conjunta discreta: probabilidades marginales e independencia
*Ej 2* — Distribución conjunta continua: constante k, marginales, E(X), V(X), independencia
*Ej 3* — fdp conjunta continua: probabilidades, marginales, covarianza

**Mié 24** — 8 ejercicios
- Ejercicios: G4 ej. 4, 5, 10, 11, 12, 13, 14, 15
*Ej 4* — Distribución conjunta con Poisson independientes: fmp conjunta, probabilidades
*Ej 5* — Distribución conjunta Exponencial independiente: fdp conjunta, probabilidades
*Ej 10* — Teorema Central del Límite (TLC): distribución de la media muestral
*Ej 11* — TLC aplicado: probabilidades de suma de tiempos independientes
*Ej 12* — Aproximación Normal a Binomial con corrección de continuidad
*Ej 13* — Aproximación Normal a Binomial: probabilidades acumuladas
*Ej 14* — Aproximación Normal a Poisson
*Ej 15* — Distribución de diferencia de medias muestrales: X̄ - Ȳ
- INV: Leer estimación puntual, estimadores insesgados, método de momentos y MV (HI-PyE-05)

**Jue 25** — 10 ejercicios
- Ejercicios: G4 ej. 16; G5 ej. 1, 2, 4, 6, 7, 8, 9; G6 ej. 1, 3
*Ej 16* — TLC con múltiples variables: distribución del promedio diario
*Ej 1* — Estimador insesgado de λ (Poisson): error estándar, comparación de estimadores
*Ej 2* — Estimación de µ, σ para dos muestras independientes: diferencia de medias
*Ej 4* — Demostraciones: X̄² no es insesgado para µ², corrección con S²
*Ej 6* — Método de momentos: estimación de θ para distribución con fdp dada
*Ej 7* — Distribución Normal: estimación por momentos y máxima verosimilitud (MV)
*Ej 8* — Distribución Exponencial: estimador MV de λ
*Ej 9* — Distribución Normal: estimadores MV de µ y σ, percentiles, P(X≤c)
*Ej 1* — IC para media con σ conocido: cálculo, longitud, tamaño muestral necesario
*Ej 3* — IC para media con n grande y σ desconocido (distribución Z aproximada)
- INV: Leer intervalos de confianza para media con σ conocido y desconocido (HI-PyE-06)

**Vie 26** — 8 ejercicios
- Ejercicios: G6 ej. 4, 5, 6, 8; G7 ej. 1, 3, 6, 9
*Ej 4* — IC para proporción: nivel 0.98, tamaño muestral
*Ej 5* — IC para media con distribución t: interpretación y conclusión
*Ej 6* — IC para media y varianza con distribución t y chi-cuadrado
*Ej 8* — IC para media y desviación estándar con t y chi-cuadrado
*Ej 1* — Prueba Z bilateral para µ con σ conocido: región de rechazo, error tipo I y II
*Ej 3* — Prueba Z bilateral: decisión al nivel 0.01, error tipo II
*Ej 6* — Prueba Z unilateral con n≥40: potencia y error tipo II
*Ej 9* — Prueba t unilateral: contenido de sodio y normas alimentarias
- INV: Leer prueba de hipótesis — H0, Ha, región de rechazo, errores tipo I y II (HI-PyE-07)

**Sáb 27** — 8 ejercicios
- Ejercicios: G7 ej. 10, 12, 15, 16; G8 ej. 1, 3, 5, 6
*Ej 10* — Prueba t bilateral + IC: lecturas de detectores de radón
*Ej 12* — Prueba Z para proporción: nivel 0.01 vs 0.05
*Ej 15* — Cálculo de p-valor desde z_obs para distintas hipótesis alternativas
*Ej 16* — Prueba t unilateral: acotación del p-valor y conclusión
*Ej 1* — Prueba Z para diferencia de medias con σ1 y σ2 conocidos: unilateral
*Ej 3* — Prueba t para diferencia de medias con varianzas iguales desconocidas
*Ej 5* — Prueba t para diferencia de medias con varianzas distintas desconocidas
*Ej 6* — Prueba t para muestras apareadas: frecuencia cardíaca antes y después
- INV: Leer pruebas con dos muestras — independientes y apareadas (HI-PyE-08)

**Dom 28** — Sin ejercicios
- Copiar y repasar ejercicios resueltos del pizarrón
- Memorizar fórmulas clave y estructura de cada tipo de prueba

**Lun 29** ⚠️ — FINAL PyE / ÁLGEBRA (1ra fecha) — sin sesión


---

## Hojas de investigación PyE

> Una hoja por tema. Fuente: Devore + apuntes de clase + búsqueda web.

- [ ] HI-PyE-01: Probabilidad condicional y Bayes — fórmula, ejemplos, árbol
- [ ] HI-PyE-02: Distribuciones discretas — tabla comparativa (Binomial, Poisson, Hipergeom.)
- [ ] HI-PyE-03: Distribuciones continuas — Normal, Exponencial, uso de tablas Z
- [ ] HI-PyE-04: TLC — enunciado, condiciones, aplicación a sumas y medias
- [ ] HI-PyE-05: Estimadores — insesgado, eficiente, consistente, método MV
- [ ] HI-PyE-06: Intervalos de confianza — Z vs T, chi-cuadrado, proporciones
- [ ] HI-PyE-07: Pruebas de hipótesis — estructura, p-valor, errores tipo I y II
- [ ] HI-PyE-08: Pruebas dos muestras — independientes vs apareadas, cuándo usar cada una

---

## Notas del método

- PyE tiene mucha teoría que hay que entender antes de hacer ejercicios: las sesiones INV son obligatorias antes de cada guía nueva
- Para G1–G4 el repaso puede ser más rápido: leer la hoja de investigación + ejercicios seleccionados
- Para G5–G8 (temas nuevos) el ciclo completo: INV → ejercicios → SR
- SR: misma caja de zapatos, compartimentos separados para PyE
- Cuando un ejercicio no sale: volver a la hoja de investigación, identificar qué fórmula o concepto falta
- PyE tiene mucho p-valor e interpretación en contexto: practicar siempre concluir en palabras, no solo calcular